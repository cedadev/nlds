        ...
        for attr in result:
            ...
            if attr["category"] == 1:
                ...
                if attr["requirements"]:
                    ...
                    raise ValueError(
                        f"Cannot find any requirements for {service_name}."
                    )
            else:
                raise ValueError(
                ...
                if requirement["resource"]["short_name"] == "tape":
                    try:
                        ...
                        if tape_quota:
                            ...
                            raise ValueError(
                                f"Issue getting tape quota for {service_name}. Quota is zero."
                            )
                    except KeyError:
                        raise KeyError(
                            f"Issue getting tape quota for {service_name}. No 'value' field exists."
                        )
                else:
                    raise ValueError(
                        f"No tape resources could be found for {service_name}"
                    )
            else:
                raise ValueError(
                    f"No provisioned requirements found for {service_name}. Check the status of your requested resources."
                )
