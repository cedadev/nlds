...
def upgrade_catalog() -> None:
    ...
    op.add_column("location", sa.Column("aggregation_id", sa.Integer(), nullable=True))
    ...
    op.create_index(
        op.f("ix_location_aggregation_id"), "location", ["aggregation_id"], unique=False
    )
    ...
    with op.batch_alter_table("location", naming_convention=naming_conv) as bop:
        ...
        bop.create_foreign_key(
            "fk_location_aggregation_id_aggregation",
            "aggregation",
            ["aggregation_id"],
            ["id"],
        )
