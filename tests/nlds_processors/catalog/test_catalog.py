...
from nlds_processors.catalog.catalog_models import (
    CatalogBase, File, Holding, Location, Transaction, Storage, Checksum, Tag
)
...
class TestCatalog:
    ...
    def test_get_holding(self, mock_catalog):
        ...
        holding = mock_catalog.get_holding(user='test-user', 
                                           group='test-group')
        ...
        with pytest.raises(CatalogError):
            holding = mock_catalog.get_holding(user='incorrect-user', 
                                               group='test-group')
        with pytest.raises(CatalogError):
            holding = mock_catalog.get_holding(user='test-user', 
        ...
        with pytest.raises(CatalogError):
            mock_catalog.get_holding(user='test-user', group='test-group', 
                                     holding_id=3)       
        # TODO: tag logic?
    ...
    def test_create_holding(self, mock_catalog):
        ...
        mock_catalog.create_holding(user='test-user', group='test-group', 
                                    label='test-label')
        # NOTE: Would be wise to directly interface with the database here to 
        # make sure the unit-test is isolated?
        holding = mock_catalog.session.query(Holding).filter(
    ...
    def test_modify_holding(self, mock_catalog):
        ...
        holding = mock_catalog.modify_holding(valid_holding, 
                                              new_label='new-label')
