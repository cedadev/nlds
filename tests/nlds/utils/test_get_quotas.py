...
def test_get_projects_services_success(monkeypatch, quotas):
    ...
    def mock_load_config():
        ...
        return {
            "authentication": {
                "jasmin_authenticator": {"user_services_url": user_services_url}
            }
        }
...
def test_get_projects_services_connection_error(monkeypatch, quotas):
    ...
    with pytest.raises(
        RuntimeError, match=re.escape(f"User services url {url} could not be reached.")
    ):
...
def test_get_projects_services_json_error(monkeypatch, quotas):
    ...
    with pytest.raises(
        RuntimeError,
        match=re.escape(f"Invalid JSON returned from the user services url: {url}"),
    ):
        quotas.get_projects_services("dummy_oauth_token", "test_service")
...
def test_extract_tape_quota_success(monkeypatch, quotas):
    ...
    monkeypatch.setattr(
        "nlds.utils.get_quotas.Quotas.get_projects_services", mock_get_projects_services
    )
