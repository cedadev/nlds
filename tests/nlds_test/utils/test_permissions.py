...
def check_modes(stat_result, user, group, outcomes=(True, True, True)):
    ...
    """
    assert outcomes[0] == check_permissions(
        user, group, access=os.R_OK, stat_result=stat_result
    )
    assert outcomes[1] == check_permissions(
        user, group, access=os.W_OK, stat_result=stat_result
    )
    assert outcomes[2] == check_permissions(
        user, group, access=os.X_OK, stat_result=stat_result
    )
