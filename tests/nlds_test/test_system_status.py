...
def mock_callback_exception(host_ip, api_port, queue_name, login, password, vhost):
    # throws a RequestException simmulating what happens if requests is offline

    raise (requests.exceptions.RequestException)


def mock_callback_request(host_ip, api_port, queue_name, login, password, vhost):
...
def test_get_consumer_status_rabbits_offline(
    monkeypatch,
    ...
    consumer = loop.run_until_complete(
        system.get_consumer_status("consumer_q", "consumer", msg_dict, time_limit, 0)
    )
    consumer = consumer[0]
...
def test_get_consumer_status_requests_failed(
    monkeypatch,
    loop: asyncio.AbstractEventLoop,
    template_config,
    load_config,
...
def test_get_consumer_status_requests_error(
    monkeypatch,
    loop: asyncio.AbstractEventLoop,
    template_config,
    load_config,
...
def test_consumer_all_offline(
    ...
    # function that is being called and store its output
    consumer = loop.run_until_complete(
        system.get_consumer_status("consumer_q", "consumer", msg_dict, time_limit, 9)
    )
    consumer = consumer[0]
...
def test_consumer_some_online(
    ...
    # function that is being called and store its output
    consumer = loop.run_until_complete(
        system.get_consumer_status("consumer_q", "consumer", msg_dict, time_limit, 2)
    )
    consumer = consumer[0]
...
def test_consumer_none_running(
    monkeypatch,
    loop: asyncio.AbstractEventLoop,
    template_config,
    load_config,
...
def test_slow_consumer_all_offline(
    monkeypatch,
    loop: asyncio.AbstractEventLoop,
    template_config,
    load_config,
...
async def mock_get_consumer_status(key, target, msg_dict, time_limit, skip_num=0):
    ...
    mock_consumer_tags = (
        {"val": ("All Consumers Online (5/5)"), "colour": "GREEN"},
        5,
        5,
    )
    return mock_consumer_tags
...
async def mock_green_consumer_status(key, target, msg_dict, time_limit, skip_num=0):
    # returns a dictionary value for get_consumer_status as a simple mock function
...
async def mock_red_consumer_status(key, target, msg_dict, time_limit, skip_num=0):
    # returns a dictionary value for get_consumer_status as a simple mock function
...
async def mock_blue_consumer_status(key, target, msg_dict, time_limit, skip_num=0):
    # returns a dictionary value for get_consumer_status as a simple mock function
...
def test_get_success(
    monkeypatch, loop: asyncio.AbstractEventLoop, load_config, load_connection
):
    ...
    status = (
        "{'monitor': {'val': 'All Consumers Online (5/5)', "
        "'colour': 'GREEN'}, "
        "'catalog': {'val': 'All Consumers Online (5/5)', 'colour': 'GREEN'}, "
        "'nlds_worker': {'val': 'All Consumers Online (5/5)', 'colour': 'GREEN'}, "
        "'index': {'val': 'All Consumers Online (5/5)', 'colour': 'GREEN'}, "
...
def test_get_alert_green(
    ...
    # replaces certain functions with mock functions that are a lot less
    # complicated and return a simple value for what is being tested
    monkeypatch.setattr(system, "get_consumer_status", mock_green_consumer_status)
...
def test_get_consumer_info_success(monkeypatch, load_config, load_connection):
    ...
    info = system.get_consumer_info(
        "server", "admin_port", key, "user", "password", "vhost"
    )
...
def test_get_service_json_success(
    monkeypatch, loop: asyncio.AbstractEventLoop, load_config, load_connection
):
    # test if get function correctly runs
